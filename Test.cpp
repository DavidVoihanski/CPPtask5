...
int main(){
    ...
    if (signal == 0) {
        ...
        .CHECK_OUTPUT(iterable_to_string(range(-3.3,2.7)), "-3.3,-2.3,-1.3,-0.3,0.7,1.7,");
        //chain
        ...
        .CHECK_OUTPUT(iterable_to_string(chain(range('A','C'),string("aba"))), "A,B,a,b,a," )
        ...
        .CHECK_OUTPUT(iterable_to_string(chain(range('a', 'e'), range('a', 'e'))), "a,b,c,d,a,b,c,d,")
        ...
        // .CHECK_OUTPUT(iterable_to_string(zip(chain(range('A', 'D'), string("ABC")), chain(range(-1.1, 1.9), range(0.3, 3.3)))), "A,-1.1, B,-0.1, C,0.9, A,0.3, B,1.3, C,2.3,")
        // ;
